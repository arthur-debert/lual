...
local function validate_field(field_name, value, field_schema, data)
...
function M.validate(data, schema, schema_registry)
    ...
    for field_name, field_schema in pairs(schema) do
        ...
        local valid, error_msg = validate_field(field_name, value, field_schema, data)
    ...
    for field_name, value in pairs(data) do
        if not schema[field_name] then
            result._errors[field_name] = "Unknown field: " .. field_name
            result.data[field_name] = value
        end
    end
