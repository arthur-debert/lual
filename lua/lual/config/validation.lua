...
function M.validate_fields(field_validations)
    for _, validation in ipairs(field_validations) do
        ...
        if constant_table then
            local valid, err = constants.validate_against_constants(value, constant_table, allow_nil, expected_type)
            if not valid then
                return false, err
            end
        end
...
function M.validate_level(level)
    ...
    if type(level) == "string" then
        ...
        return constants.validate_against_constants(level, constants.VALID_LEVEL_STRINGS, false)
...
function M.validate_timezone(timezone)
    return constants.validate_against_constants(timezone, constants.VALID_TIMEZONES, true, "string")
end
