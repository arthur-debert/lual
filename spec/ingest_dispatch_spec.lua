...
describe("ingest.dispatch_log_event", function()
  ...
  it("should call output and formatter for a single logger", function()
    set_mock_loggers({
      main_logger = create_mock_logger("main_logger", _G.log.levels.INFO, {
        { formatter_func = mock_formatter_func, output_func = mock_output_func, output_config = { dest = "mock_output" } }
      })
    })
    ...
    if #logger.outputs > 0 then
      print("Output func is mock_output_func: " ..
        tostring(logger.outputs[1].output_func == mock_output_func))
    end
  ...
  it("should propagate from child to parent, both processing", function()
    local parent_logger = create_mock_logger("parent_logger", _G.log.levels.DEBUG, {
      { formatter_func = mock_formatter_func, output_func = mock_output_func, output_config = { id = "parent_h" } }
    })
    local child_logger = create_mock_logger("child_logger", _G.log.levels.DEBUG, {
      { formatter_func = mock_formatter_func, output_func = mock_output_func, output_config = { id = "child_h" } }
    }, true, parent_logger)
  ...
  it("should handle formatter error gracefully and use fallback message", function()
    set_mock_loggers({
      error_logger = create_mock_logger("error_logger", _G.log.levels.INFO, {
        { formatter_func = mock_erroring_formatter_func, output_func = mock_output_func, output_config = { id = "h_after_fmt_err" } }
      })
    })
  ...
  it("error in one output should not affect subsequent outputs for the same logger", function()
    set_mock_loggers({
      multi_output_logger = create_mock_logger("multi_output_logger", _G.log.levels.INFO, {
        { formatter_func = mock_formatter_func, output_func = mock_erroring_output_func, output_config = { id = "error_output" } },
        { formatter_func = mock_formatter_func, output_func = mock_output_func_ok,       output_config = { id = "ok_output" } }
      })
    })
  ...
  it("error in child's output should not affect propagation to parent", function()
    local parent_logger_prop = create_mock_logger("parent_logger_prop", _G.log.levels.INFO, {
      { formatter_func = mock_formatter_func, output_func = mock_output_func_ok, output_config = { id = "parent_ok_h" } }
    })
    local child_logger_prop_error = create_mock_logger("child_logger_prop_error", _G.log.levels.INFO, {
      { formatter_func = mock_formatter_func, output_func = mock_erroring_output_func, output_config = { id = "child_err_h" } }
  ...
  it("should pass all event_details fields correctly to formatter and output", function()
    ...
    local passthrough_logger = create_mock_logger("passthrough_logger", _G.log.levels.INFO, {
      { formatter_func = mock_formatter_func, output_func = mock_output_func, output_config = { id = "passthrough_h" } }
    })
