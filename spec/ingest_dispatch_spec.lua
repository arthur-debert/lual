...
describe("ingest.dispatch_log_event", function()
	...
	local function create_mock_logger(name, level, dispatchers, propagate, parent, timezone)
		local logger = {
			name = name,
			level = level,
			...
			propagate = propagate,
			parent = parent,
			timezone = timezone or "local", -- Add timezone support
					...
					if current.dispatchers then
						for _, h_entry in ipairs(current.dispatchers) do
							...
							table.insert(collected_dispatchers, {
								...
								owner_logger_timezone = current.timezone, -- Add timezone context
							})
						end
					end
