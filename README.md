...
loggers, multiple log levels, configurable handlers, and custom message
formatting.
...
- **Stream Handler:** `lualog.handlers.stream_handler` writes log messages to
...
- **Plain Text Formatter:** `lualog.formatters.plain_formatter` formats messages
  ...
  in UTC).
- **Per-Logger Configuration:** Log levels and handlers (with their formatters)
...
- **Message Propagation:** Log messages processed by a logger are passed to its
  parent's handlers by default. Propagation can be disabled per logger
...
- **Contextual Information:** Log records automatically include a UTC timestamp,
  ...
  emitted.
- **Error Handling:** Errors within handlers or formatters are caught and
  reported to `io.stderr`, preventing the logging system from crashing the
  application.
- **Default Setup:** On require, a root logger is configured with a stream
  handler (outputting to `io.stdout`) and the plain formatter, set at `INFO`
  level.
...
1.  Copy the `lua/lual` directory (containing `logger.lua`, `ingest.lua`, and
    subdirectories like `core/`, `handlers/`, `formatters/`) into your project's
...
- This sets up the **root logger** with:
  ...
  - One handler: `lualog.handlers.stream_handler` writing to `io.stdout`.
  - Formatter for this handler: `lualog.formatters.plain_formatter`.
...
- Has no handlers directly attached to it (unless you add them).
- Will propagate messages to its parent logger's handlers (ultimately to the
  root logger if propagation is enabled all the way up).
...
app_logger:info("This will be printed to stdout via the root logger's default handler.")
...
handlers (though they might still be processed by ancestor handlers if the
...
-- No need to add a handler to see output if root logger is handling it.
...
Handlers determine what happens to a log record. Each logger can have multiple
handlers. Adding a handler to a specific logger allows for output independent
of, or in addition to, propagated messages.
...
-- Handler 1: Log specifically from data_processor_logger to stderr
data_processor_logger:add_handler(
  lualog.handlers.stream_handler,
  lualog.formatters.plain_formatter,
  { stream = io.stderr }               -- Handler-specific config
)

-- Handler 2: Log to a file (using stream_handler)
-- Note: A dedicated file_handler that manages file opening/closing based on a path in config is a planned future enhancement.
local file_handle = io.open("data_processor.log", "a")
if file_handle then
  data_processor_logger:add_handler(
    lualog.handlers.stream_handler,      -- Use stream_handler for file streams too
    lualog.formatters.plain_formatter,
    { stream = file_handle }             -- Specify the file stream
  )
else
  data_processor_logger:error("Could not open data_processor.log for logging.") -- This goes to stderr and propagates to root (stdout)
end

data_processor_logger:info("Processing started.")
-- This message goes to:
-- 1. stderr (via data_processor_logger's own first handler)
-- 2. data_processor.log (via data_processor_logger's own second handler)
-- 3. stdout (via propagation to the root logger's default handler)
```

The `add_handler` method for a logger instance takes:

1.  `handler_func`: e.g., `lualog.handlers.stream_handler`.
2.  `formatter_func`: e.g., `lualog.formatters.plain_formatter`.
3.  `handler_config` (optional table).

Global `lualog.add_handler(logger_name, ...)` is also available but currently
...
- **Custom Handler:** A function with the signature `my_handler(record, config)`
  - `record`: A table containing the log details. Key fields include:
    ...
    - `logger_name`: Name of the logger that owns the handler processing the
      record.
...
- Dedicated `file_handler` that takes a filepath in configuration and manages
...
- More sophisticated handler types (e.g., network, syslog, rotating file).
